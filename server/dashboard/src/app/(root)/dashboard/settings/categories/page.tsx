...
export default function CategoriesPage() {
  ...
  const settings = useProjectSettings("categories");
  ...
  const toggleAutoAdd = async (categoryId: string, autoAdd: boolean) => {
    setBusy(true);
    try {
      await api.patch(CATEGORY_ENDPOINTS.BY_ID(categoryId), {
        auto_add: autoAdd,
      });
      toast({
  ...
  return (
    <div className="space-y-6">
      <SettingsHeader
        title="Categories"
        description={`Labels applied to memories in ${scope?.project_name ?? "this project"}. Categories belong to a project, so two projects can keep different taxonomies.`}
      />

      {!editable && <ReadOnlyNotice role={scope?.role ?? "reader"} />}

      <SettingsSection
        title="Classification"
        description="How new memories get their categories."
      >
        <SettingsToggleRow
          label="Classify new memories automatically"
          description="On, the model picks from the active categories below as each memory is written. Off, categories are only applied by hand or by an auto-add rule."
          control={
            <Switch
              checked={settings.draft.auto_classify}
              disabled={!editable}
              onCheckedChange={(v) => {
                settings.set("auto_classify", v);
              }}
              aria-label="Classify new memories automatically"
            />
          }
        />
        <SettingsSaveBar
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-onSurface-default-secondary mt-1">
            New memories are auto-classified against this list; you can correct
            any assignment by hand.
          </p>
        </div>
