...
For this private fork, use `server/.env` for local development and `server/.env.production` on the VPS. Real env files are intentionally ignored by git. Fill in `OPENAI_API_KEY` before creating memories; the server refuses to start if `JWT_SECRET` is unset once auth is enabled.
